...
<head>
    ...
    <title>UCC-CS | Forgot Password</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@latest/tabler-icons.min.css">
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Inter', sans-serif; min-height: 100vh; background: #1a6b3a; display: flex; align-items: center; justify-content: center; padding: 1rem; position: relative; }
        ...
        .card-wrap { position: relative; z-index: 1; display: flex; width: 100%; max-width: 900px; min-height: 520px; border-radius: 16px; overflow: hidden; box-shadow: 0 24px 64px rgba(0,0,0,0.25); }
        ...
        .steps-guide li { display: flex; align-items: flex-start; gap: 12px; font-size: 13px; color: rgba(255,255,255,0.8); margin-bottom: 1rem; }
        .step-num { width: 22px; height: 22px; border-radius: 50%; background: rgba(255,255,255,0.15); border: 1px solid rgba(255,255,255,0.3); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; flex-shrink: 0; margin-top: 1px; }

        /* RIGHT */
        .right-panel { flex: 1; background: #fff; padding: 2.8rem 2.5rem; display: flex; flex-direction: column; justify-content: center; }
        .brand-row { display: flex; align-items: center; gap: 10px; margin-bottom: 2rem; }
        .brand-icon { width: 40px; height: 40px; background: #1a56db; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 18px; }
        ...
        .btn-send { padding: 11px 16px; background: #1a56db; color: #fff; border: none; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; white-space: nowrap; font-family: 'Inter', sans-serif; transition: background 0.2s; flex-shrink: 0; }
        .btn-send:hover { background: #1648c0; }
        ...
        .verified-badge { display: none; align-items: center; gap: 8px; background: #eff6ff; border: 1.5px solid #1a56db; border-radius: 8px; padding: 10px 14px; font-size: 13px; color: #1a56db; font-weight: 500; margin-bottom: 1rem; }
        ...
        .btn-primary { width: 100%; padding: 13px; background: #1a56db; color: #fff; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; font-family: 'Inter', sans-serif; transition: background 0.2s; margin-top: 1rem; display: flex; align-items: center; justify-content: center; gap: 8px; }
        ...
        .modal-box { background: #fff; border-radius: 16px; padding: 2rem; width: 100%; max-width: 420px; box-shadow: 0 24px 64px rgba(0,0,0,0.2); text-align: center; }
        .modal-icon { width: 64px; height: 64px; border-radius: 50%; background: #eff6ff; display: flex; align-items: center; justify-content: center; margin: 0 auto 1.2rem; font-size: 28px; color: #1a56db; }
        ...
        .modal-tips { background: #f9f9f9; border-radius: 10px; padding: 1rem; text-align: left; margin-bottom: 1.5rem; }
        .modal-tips p { font-size: 12px; color: #555; margin-bottom: 0.5rem; display: flex; align-items: flex-start; gap: 8px; line-height: 1.5; }
        .modal-tips p i { color: #1a56db; font-size: 14px; flex-shrink: 0; margin-top: 1px; }
        .btn-got-it { width: 100%; padding: 13px; background: #1a56db; color: #fff; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; font-family: 'Inter', sans-serif; transition: background 0.2s; }
        .btn-got-it:hover { background: #1648c0; }

        @media(max-width:640px) { .card-wrap { flex-direction: column; } .left-panel { padding: 2rem; min-height: 180px; } .right-panel { padding: 2rem 1.5rem; } }
    </style>
</head>
<body>
...
<div class="card-wrap">
    ...
    <div class="left-panel">
        ...
        <ul class="steps-guide">
            <li><div class="step-num">1</div> Enter your registered UCC-CS email address.</li>
            <li><div class="step-num">2</div> Check your email for the 6-digit verification code.</li>
            <li><div class="step-num">3</div> Enter the code and set your new password.</li>
        </ul>
    </div>
    ...
    <div class="right-panel">
        ...
        <div class="brand-row">
            ...
            <div><div class="brand-name">UCC-CS</div><div class="brand-sub">Consumable Management System</div></div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="otp-row">
                <div class="input-wrap">
                    ...
                    <input type="email" class="form-control" id="fp-email" placeholder="Enter your email">
                </div>
                <button class="btn-send" id="btn-send-code" onclick="sendCode()">Send Code</button>
            </div>
            <div class="hint" id="email-hint"></div>
            <div class="timer" id="fp-timer" style="display:none">Resend code in <span id="fp-timer-count">60</span>s</div>
        </div>
        ...
        {{-- PASSWORD (shows after OTP verified) --}}
        <div id="password-section">
            <div class="verified-badge show"><i class="ti ti-circle-check"></i> Email verified — now set your new password.</div>
            <div class="form-group">
                <div class="form-label"><i class="ti ti-lock"></i> New Password</div>
                <div class="input-wrap" style="position:relative;">
                    <i class="ti ti-lock input-icon"></i>
                    <input type="password" class="form-control" id="fp-password" placeholder="Minimum 8 characters" oninput="checkStrength()" style="padding-right:40px;">
            ...
            <div class="form-group">
                ...
                <div class="input-wrap" style="position:relative;">
                    ...
                    <input type="password" class="form-control" id="fp-confirm" placeholder="Re-enter your new password" oninput="checkMatch()" style="padding-right:40px;">
        ...
        <div class="back-link">Remembered your password? <a href="{{ route('login') }}">Sign In →</a></div>
...
<div class="modal-overlay" id="reminder-modal">
    <div class="modal-box">
        ...
        <div class="modal-desc">Your password has been updated. Here are some tips to keep your account secure.</div>
        <div class="modal-tips">
            ...
            <p><i class="ti ti-eye-off"></i> Never share your password with anyone.</p>
        </div>
        <button class="btn-got-it" onclick="goToLogin()">Got it — Sign In Now</button>
    </div>
</div>
...
async function sendCode() {
    ...
    try {
        const res  = await fetch('{{ route("password.send") }}', { method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF}, body:JSON.stringify({email}) });
        const data = await res.json();
        if (!res.ok) { setHint(hint, data.errors?.email?.[0] || data.message, 'error'); btn.disabled = false; btn.textContent = 'Send Code'; return; }
    ...
    } catch (e) { setHint(hint, 'Something went wrong. Try again.', 'error'); btn.disabled = false; btn.textContent = 'Send Code'; }
}
...
function startTimer(btn) {
    ...
    timerInterval = setInterval(() => {
        ...
        if (secs <= 0) { clearInterval(timerInterval); timerEl.style.display = 'none'; btn.disabled = false; btn.textContent = 'Resend Code'; document.getElementById('fp-email').disabled = false; }
    }, 1000);
}
...
async function verifyCode() {
    ...
    try {
        const res  = await fetch('{{ route("password.verify") }}', { method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF}, body:JSON.stringify({email, code}) });
        const data = await res.json();
        if (!res.ok) { setHint(hint, data.message, 'error'); btn.disabled = false; btn.innerHTML = '<i class="ti ti-shield-check"></i> Verify Code'; return; }
        verifiedEmail = email; verifiedCode = code;
    ...
    } catch (e) { setHint(hint, 'Something went wrong. Try again.', 'error'); btn.disabled = false; btn.innerHTML = '<i class="ti ti-shield-check"></i> Verify Code'; }
}
...
async function resetPassword() {
    ...
    if (pass !== conf)   { setHint(hint, 'Passwords do not match.', 'error'); return; }
    btn.disabled = true; btn.innerHTML = '<i class="ti ti-loader-2"></i> Resetting...';
    try {
        const res  = await fetch('{{ route("password.reset") }}', { method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF}, body:JSON.stringify({email:verifiedEmail, code:verifiedCode, password:pass, password_confirmation:conf}) });
        const data = await res.json();
        if (!res.ok) { setHint(hint, data.message, 'error'); btn.disabled = false; btn.innerHTML = '<i class="ti ti-lock-check"></i> Reset Password'; return; }
        document.getElementById('reminder-modal').classList.add('open');
    } catch (e) { setHint(hint, 'Something went wrong. Try again.', 'error'); btn.disabled = false; btn.innerHTML = '<i class="ti ti-lock-check"></i> Reset Password'; }
}

function goToLogin() { window.location.href = '{{ route("login", ["reset" => true]) }}'; }
